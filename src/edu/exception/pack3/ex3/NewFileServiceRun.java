...
public class NewFileServiceRun {
    // 자바는 컴퓨터와 직접적으로 연관이 있는 서버언어로
    // 파일 생성, 삭제, 수정과 같이 직접적으로 영향을 주는
    // 코드 작업이 가능
    ...
    public static void main(String[] args) {
        ...
        // 파일을 생성하기
        // 파일만들기 할 때 덮어쓰기가 자동으로 되므로 존재유무를 확인하고
        // 퍼알명칭이 존재하면(1) 이런식으로 뒤에 번호를 붙임
        File file = new File("새 택스트 문서.txt");
        if (!file.exists()) {//생성된.txt가 존재하지 않는게 사실이라면
            try {
                file.createNewFile();
                System.out.println("file 생성이 완료되었습니다.");
            } catch (IOException e) {
                System.out.println("file을 생성하는 도중 문제가 발생했습니다.");
            } catch (Exception e) {
                System.out.println("개발자가 애기치 못한 문제가 발생했습니다.");
