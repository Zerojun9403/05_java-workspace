...
/*
       ...
       프론트 엔드에서 예외처리를 하더라도 백엔드에서도 회사나 프로그램이 진행하고자 하는 방향과
       다른 상황이 발생했을 경우, 프로그램이 진행되지 않도록 백엔드에서도 예외처리를 진행하여
       기능을 중지하는 상황을 만들어줘야함

       프로그램에서 원하는 상황으로 진행할 수 있도록 개발자가 프로그램 흐름 한 번더 설정
*/
public class ExceptionPractice2 {
    ...
    public void problem1() {
        ...
        try {
            int age = sc.nextInt();
            // 나이를 입력받아서 0보다 작거나 150보다 크면 직접 예외를 발생시키세요 throw new 작성해서 0~150 이상이어야 합니다.
            if (age < 0 || age > 150) {
    ...
    public void problem4() {
        ...
        try {
            ...
            if (!email.contains("@") || !email.contains(".")) {
                throw new IllegalArgumentException("이메일에는 '@'와 '.'이 반드시 포함되어야 합니다.");
            }
