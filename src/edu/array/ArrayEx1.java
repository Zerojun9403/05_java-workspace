...
public class ArrayEx1 {
    ...
    /* 배열 (자료구조)
     - java에서의 배열
      ...
      - 생성된 배열을 활욯하기 위해서는
         배열 참조하는 "참조형 변수"를 이용한다.

         [참조형 변수]
         - 주소를 정하는 변수
         - 변수 사숑잇 지정된 주소를 찾아가서 그 곳에 위치한 배열, 객체를 참조한다.,
    ...
    * */
    ...
    public void method1() {
        ...
        int[] arr;
        // Stack 영역 안에 int[] 자료형을 참조할 변수를 생성하고(주소 저장)
        // 그 변수 이름을 "arr" 으로 지정


        arr = new int[4];
        // Heap 영역에 int 4개 짜리 배열 할당 후

        // 생성된 배열의 주소를 "arr" 변수에 대입

        //-> arr 을 이용해서 배열을  참조해서 arr에 저장된 데이터를 사용할 수 있음


        // 배열 초기화
        // 1) 초기화 전 -> 0 이 대입되어 있는지 확인
        System.out.println("초기화 전 배열의 상태 ");
        System.out.println("arr 가 위치한 컴퓨터 내의 주소명칭 : " + arr);
        ...
        *   오류내용
	    ...
	    at edu.array.ArrayEx1.method1(ArrayEx1.java:62)
	    at edu.array.ArrayRun.main(ArrayRun.java:9)
        Index 4 out = index 배열이 배치될 수 있는 칸은 4칸인데 4칸인상을 작성해서 발생하는 예외상황
        자바스크립에서는 length 의 길이도 데이터가 추가되면 자연스럽게 length의 길이도 커지지만
        자바는 최초로  지정한 칸 수를 그대로 유지하며, 칸 수를 초과할 경우 예외상황 발생

        추후 ArrayList 로 자바스크립트 처럼 최초로 지정한 칸 수에 관계 없이 데이터를 추가할 수 있는
        class 파일을 자바 개발자가 만든 후 자바 버전 업그레이드 하였고,
        개발자들은 ArrayList 와 같은 기능을 활용해서 유연하게 데이터를 추가하고 삭제할수 있음
        ...
        * */
        ...
        /*
         *  디버그 모드 : 확인하고 싶은 코드 다음 줄 break Point 추가해서 문제 확인할 것 !
         *
         * */
    }
    ...
    * for 문을 활용해서 배열 값 확인 */
    ...
    public void  method2() {
        ...
        // index 는 length -1 한 값
        // index 0 번 부터 length -1 번 까지만 추가할것
