 ...
 *
 * 기본 구조
 * 접근제한자 [static] 반환 타입 메서드명칭(매개변수=파라미터){
 *   // 실행할코드
 *   return 반환타입에 해당하는 반환값 데이터;
 * }
 *
 * 접근제한자 [static] void 타입 메서드명칭(매개변수=파라미터){
 * // 실행할코드
 *
 * }
 *
 ...
 *
 * 클래스 매사드(static Method) / 클래스 변수 (Static Variable)
 * static 키워드가 붙은 메서드
 * 객체 생성없이 클래스명칭으로 직접 호출
 * 모든 객체가 공유하는 기능
 *
...
public class MethodEx {
    ...
    public boolean isPopularCafe() {
        ...
        boolean result = totalOders >= 50;

        return result;
        // return totalOders >= 50;
    }


    // 5. static 메서드( = class 메서드)
    //     클래스파일명칭 .static 기능명칭(매개변수 존재 유무);
    public static void systemInfo() {
        // 모든 객체들이 알아야할 컴퓨터의 기본 정보 작성
    }

    // 나중에 systemInfo 기능을 사용할때는
    // MethodEx ex = new MethodEx();
    // ex.systemInfo(); 형식으로 작서하는 것이 아니라
    // MethodEx.systemInfo(); 형식으로 작성해야한다.!!!


    /* 6.메서드 오버로딩(Method Overloading)
    하나의 클래스 내부에서 이름이 동일한 메서드를 여러개 정의
    같은 이름으로 새로운 메서드를 계속 추가하는 것

    오버로딩 조건
    메서드 이름은 같아야 하지만, 매개변수는 달라야 함
    1. 매개변수의 자료형 개수가 다른경우
    2. 매개변수의 개수는 다른경우
    ...
    // SNS로그인 -> SNS 회사에서는 진짜 아이디와 비밀번홀르 외부 개발자에게 제공하지 않음
    // 대신 SNS 본인인증을 마쳤으니 믿고 입장해도 좋다 의미로 토큰을 제공
    // 축제나 클럽에 들어갈때 입구에서 신분증을 보여주고 본인인증을 하면
    // 다시는 신분증을 꺼낼 필요없이 팔찌만 보여주고 자유롭게 다닐 수 있는거 처럼
    // 온라인에서는 팔찌 대신  토큰을 제공
    // 6-2 소셜 로그인 토큰(String)으로 로그인
    ...
    // 6-3 유저아이디(int)와 비밀번호(Strig)으로 로그인
