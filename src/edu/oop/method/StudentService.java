...
public class StudentService {
    ...
    public void displayMenu() {
        ...
        while (true) {
            ...
            // Scanner 는 사용하지 않고, System 은 개발자가 데이터를 무사히 전달하고, 사용하고 있는지 확인 용도
            System.out.println("""
                    === 학생 관리 프로그램 ===
                    1. 학생 등록
                    2. 학생 정보 확인
            ...
            switch (input) {
                case 1: // register.html 에서 학생 등록하는 태그를 작성
                    ...
                    if (select == 1) {
                        ...
                        // 학생 등록을 위한 메서드 호출
                        std2 = createStudent(); // return 으로 가져올 아래 생성자 객체 대신 기능명칭 작성
                        //std2 = new Student(createName, createNumber, createGender);
                        studentInform(std2); // 등록이 잘 되었는지 학생 정보 확인
                    }
                    break;
                case 2: // mypage.html
     ...
     *
     * @param student : std1 또는 std2 (학생의 대한 정보가 담겨있는 객체 형태 위치 주소를 가져오기)
     *                <p>
    ...
    private void updateJava(Student student) {
        ...
        int result = newJava + student.getJava(); // -숫자값을 입력하면 자동으로 +- 에서 -로 처리
        ...
        while (result > Student.MAX_VALUE || result < Student.MIN_VALUE) {
            ...
            newJava = sc.nextInt();
            result = newJava + student.getJava();
    ...
    private void updateHtml(Student student) {
        ...
        int result = newHtml + student.getHtml(); // -숫자값을 입력하면 자동으로 +- 에서 -로 처리
        ...
        while (result > Student.MAX_VALUE || result < Student.MIN_VALUE) {
            ...
            System.out.println("다시 입력해주세요 : ");
            newHtml = sc.nextInt();
            result = newHtml + student.getHtml();
            // 입력 종료를 원하시면 000 를 작성해주세요.  000 숫자를 입력하면 return;
        }
    ...
    /* 매개 뱐수로 전달 받은 우 Student 의 Java 점수 비교
    * */
