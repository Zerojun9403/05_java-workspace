 ...
 *  문서를 추후에 new 클래스 이름() 생성자를 이용하여 객체로 호출
 * */
public class Nation {

    // private 대신 public을 이용해서 다른 클래스 파일에서 Nation에 작성된 속성 기능 호출해서 사용
    ...
    * */
    ...
    /*
    *  set 변수 이름을 직접적으로 활용하지 않으면
    * setAge 메서드 이름 처럼 회색빛 으로
    * 사용되지 않은 매서드 임이 표기되어 있음
    * */
    ...
    /* *********************
    *  Getter 를 활용하는 경우
    *  1. 다른 클래스에서 정보가 필요할 떄
    *  2. 매개변수로 값을 전달할때
    *  3. 조건문ㅇ에서 값을 비교할때
    *  4. 계산 작업할 떄
    *  -----------------
    * 현재 getter 를 활용해서 setter에 저장된 데이터를 출력하지만
    * DB에 저장된 값을 전달하는 용도
    * 회원가입시 이메일 인증번호 일치 여부 와 같이 특정 데이터에 대한 값 확인
    * 주로 프론트앤드로 DB에 존재하는 데이터 전달할 떄 많이 사용
    *
    * 대표적인 예)
    * 수정하기  -> DB에서 수정해야하는 게시물 번호 get 가져오기
    *          -> 게시물 번호에 해당하는 데이터를 get 으로 전달하여 프론트엔드에서 JS로 전달받아
    ...
    *  ********************* */
    ...
    public void introduce() {
        System.out.printf("이름 %s이고 , %d세 %c성입니다. \n", name, age, gender);
    }
}
