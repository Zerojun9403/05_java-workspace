...
public class FileEx {
    /*
        - 파일 읽기 관련 -
        FileInputStream : 파일의 바이트 데이터를 읽어 오는 클래스
        FileReader : 파일의 텍스트 데이터를 문자 단위로 읽는 클래스
        BufferedReadr : 파일의 텍스트를 버퍼에 담아 효율적으로 읽는 클래스
        ...
        - 파일 정보 및 관리 -
        File : 파일이나 폴더의 존재여부, 정보 확인하는 레거시 클래스
        Files : 파일 생성, 삭제, 복사 등 모든 파일 작업을 담당하는 유틸리티 클래스
        Path : 파일경로를 나타내고 조작하는 현대적 클래스
        Paths ; Path 객체를 생성하는 팩토리 클래스

       * 레거시(Legacy) : 오래된 예전 방식의 구식의 전통방식
    ...
    * */
    ...
    public void legacyMethod1() {
        ...
        if (!success) {
            // 삭제를 못한게 사실이라면
            // 왜 삭제를 못했는가? 권헌이 없어서? 다른 프로그램에서 사용중인지?
            // 개발자가 코드를 수정함에 있어 어떻해 문제를 수정하고 대안을 설정해야하는지에 대한 정보가 없음
        ...
        try {
            ...
            System.out.println("파일이 없습니다 : -> 파일이 없을 떄 어떻게 진행햐햐는지에 대한 개발자가 작성");
        } catch (AccessDeniedException e) {
            System.out.println("권한이 없습니다 : -> 이런 경우 개발자가 어떻헤 처리해야하는지 진행");
        } catch (IOException e) {
            System.out.println("입 출력 연결오류 : -> 개발바가 어떻게 처리해애하는지 진행");
        } catch (Exception e) {
    ...
    public void factorMethod() {
        ...
        File flie1 = new File("폴더경로/파일이름.확장자이름");
        File flie2 = new File("폴더경로", "/파일이름.확장자이름");

        // 현대 버전 Windows 랑 MacBook Linux 을 자바 코드에서 운영체제를 읽고, 사이 중간을 알아서
        // Windows 랑 MacBook Linux 에 각각 필요한 형태로 / 를 작성
        // windows 에서 User/user/file.text
        //
        // Paths 팩토리의 겨웅
        Paths path1 = Paths.get("file.txt"); //파일만 필요할때 사용
        Paths path2 = Paths.get("c: ", "폴더이름", "file.txt"); //최상위폴더/ 중간폴더/ 파일명

        //Java 11 버전 부터 사용되는 초신 방법
        // Paths 와 Path 구분하지 못한채 사용하는 경우가 많아 Path라는 명칭으로 통일해서
        // 폴더이름 파일이름.확장자이름 의 경로를 사용할 수 있도록 설정된 기능
        Paths path3 = Path.of("file.txt");

        // 절대 경로(전체적인 경로)
        Path 절대경로1 = Path.of("C:","User","user","Desktop","file.txt");
